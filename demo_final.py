...
ok(f"Integridad de cadena local: {'✓ válida' if validez['valid'] else '✗ inválida'}")

if bal_carlitos == 600 and bal_emilio == 250 and bal_carlos == 150 and bal_gael == 100:
    print(f"  {GREEN}{BOLD}  ✓ Saldos correctos ✓{RESET}")
else:
...
if len_a == len_b:
    print(f"\n  {GREEN}{BOLD}✓ Ambos nodos sincronizados — longitud {len_a}{RESET}")
else:
    print(f"\n  {RED}✗ Desincronizados: A={len_a}, B={len_b}{RESET}")

print(f"\n{YELLOW}{'='*60}{RESET}")
print(f"{BOLD}{YELLOW}  🧱  CryptoStuds — Demo Completada{RESET}")
print(f"{YELLOW}{'='*60}{RESET}")
print(f"""
  {GREEN}✓{RESET} Blockchain con PoW (dificultad 4 ceros)
  {GREEN}✓{RESET} Árbol de Merkle en cada bloque
  {GREEN}✓{RESET} Wallets con par de claves secp256k1
  {GREEN}✓{RESET} Transacciones firmadas con ECDSA
  {GREEN}✓{RESET} Validación de firma y prevención de doble gasto
  {GREEN}✓{RESET} API REST con Flask
  {GREEN}✓{RESET} Red P2P con consenso de cadena más larga
  {GREEN}✓{RESET} Propagación automática de bloques
""")
